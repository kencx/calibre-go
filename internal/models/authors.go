...
var AuthorWhere = struct {
	...
	ID:   whereHelpernull_Int64{field: "\"authors\".\"id\""},
	Name: whereHelperstring{field: "\"authors\".\"name\""},
	Sort: whereHelpernull_String{field: "\"authors\".\"sort\""},
	Link: whereHelperstring{field: "\"authors\".\"link\""},
}
...
var (
	authorAllColumns            = []string{"id", "name", "sort", "link"}
	authorColumnsWithoutDefault = []string{"name"}
	authorColumnsWithDefault    = []string{"id", "sort", "link"}
	authorPrimaryKeyColumns     = []string{"id"}
	authorGeneratedColumns      = []string{"id"}
)
...
func (o *Author) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, authorGeneratedColumns)
...
func (o *Author) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"authors\"", updateOnConflict, ret, update, conflict, insert)
