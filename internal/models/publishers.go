...
var PublisherWhere = struct {
	...
	ID:   whereHelpernull_Int64{field: "\"publishers\".\"id\""},
	Name: whereHelperstring{field: "\"publishers\".\"name\""},
	Sort: whereHelpernull_String{field: "\"publishers\".\"sort\""},
	Link: whereHelperstring{field: "\"publishers\".\"link\""},
}
...
var (
	publisherAllColumns            = []string{"id", "name", "sort", "link"}
	publisherColumnsWithoutDefault = []string{"name"}
	publisherColumnsWithDefault    = []string{"id", "sort", "link"}
	publisherPrimaryKeyColumns     = []string{"id"}
	publisherGeneratedColumns      = []string{"id"}
)
...
// CountG returns the count of all Publisher records in the query using the global executor
...
func FindPublisher(ctx context.Context, exec boil.ContextExecutor, iD null.Int64, selectCols ...string) (*Publisher, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"publishers\" where \"id\"=?", sel,
	)
...
func (o *Publisher) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, publisherGeneratedColumns)
...
func (o *Publisher) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"publishers\"", updateOnConflict, ret, update, conflict, insert)
...
func (q publisherQuery) DeleteAllG(ctx context.Context) (int64, error) {
...
func PublisherExists(ctx context.Context, exec boil.ContextExecutor, iD null.Int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"publishers\" where \"id\"=? limit 1)"
